# services/portfolio/app_main.py
# FastAPI app for GSA Portfolio (admin endpoints + normalize)
# Uses DISTINCT ON (..., fetched_at DESC) to fix the earlier ORDER BY error.
...
async def normalize_from_raw(
    ...
    Dedup with DISTINCT ON (game_id, sport_key, payload_hash), picking latest by fetched_at.
    """
    await _ensure_pool_open()

    # Source rows — FIXED with DISTINCT ON + ORDER BY fetched_at DESC
    async with pool.connection() as ac, ac.cursor(row_factory=dict_row) as cur:
        await cur.execute(
            """
